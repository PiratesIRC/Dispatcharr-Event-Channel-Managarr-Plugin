...
| **💤 Regex: Mark Channel as Inactive** | `text` | - | Regular expression to hide channels. Processed as part of the `[InactiveRegex]` hide rule. For example, `CANCELL?ED` hides a channel whose name contains CANCELLED or CANCELED, in any case. Before v1.26.2621024 a backslash sequence such as `\b` (word boundary) never matched in this one field, so a pattern like `\bCANCELLED\b` hid nothing; it now behaves as it does in the other two regex fields. |
...
**All three regex fields above are matched against the Dispatcharr channel name, whatever the Name Source setting is set to.** Those three fields pick out a particular channel to skip, hide or keep visible, and you write them while looking at the channel list, so they do not follow **Name Source**; the stream bound to a channel can also be replaced on any M3U refresh. Every other hide rule does read the **Name Source** text. Earlier versions matched these three fields against the **Name Source** text too, so setting **Name Source** to `Stream_Name` made a pattern written for channel names quietly stop matching. They are never matched against guide programme titles or descriptions, so text copied out of the TV Guide will not match anything. Separate alternatives with `|`, for example `NFL REDZONE|NFL NETWORK`. A regex field that you have filled in but that matched no channels is reported in the result message and counted in the CSV header under **Regex Field Matches**, so a pattern that can never fire is visible rather than looking like one that works.
...
| Setting | Type | Default | Description |
| :--- | :--- | :--- | :--- |
| **🎭 Duplicate Handling Strategy** | `select` | `lowest_number` | Which copy stays visible when several channels carry the same event: **Keep Lowest Channel Number**, **Keep Highest Channel Number** or **Keep Longest Channel Name**. A blank value behaves as Keep Lowest Channel Number. You cannot name one particular channel as the one to keep. |
...
| **🗂️ Per-Group EPG Sources (one per line)** | `text` | *(empty)* | Gives a channel group its own dummy EPG source, one mapping per line as `Group Name = Source Name`. A group you do not list keeps the shared source, so leaving this blank changes nothing. See [Per-Group EPG Sources](#per-group-epg-sources) below. (new in v1.26.2451734) |
...
| **[UndatedAge:days]** | `days` (int) | Hides channels whose names contain **no parseable date** once they've been visible for more than `days` days. Persists per-channel first-seen state in `/data/event_channel_managarr_undated_first_seen.json`. Resets a channel's age when its name changes. Days are calendar days in Dispatcharr's time zone, so `[UndatedAge:0]` hides an undated channel from the day after it was first seen, not on the same day. A low number also hides a live game whose name carries no date, because the rule has nothing in the name to tell it the game is on. |
| **[UndatedEnded]** or **[UndatedEnded:hours]** | optional `hours` (int) | Hides a channel whose name carries a **clock time but no date** once that event's inferred end has passed. The window starts on the day the name states, when it states one: a name carrying a full day of the week, or the football markers `MNF`, `TNF` or `SNF`, is anchored to the first such day on or after the date the channel was first seen, so a slate published on a Thursday for a Monday night game is not treated as a Thursday event. A name stating no day uses the first-seen date itself (the same `/data/event_channel_managarr_undated_first_seen.json` record `[UndatedAge:days]` uses). To that start the rule adds the time read from the name, plus the event duration and time pattern taken from the dummy EPG source the channel is bound to (falling back to the **Event Duration** and **Channel Name Event Timezone** settings), plus the grace period. Without a number it reads the **Undated Event Grace Period (Hours)** setting. Every step fails open: a channel with no first-seen record, no readable time or an unusable timezone stays visible and is left to `[UndatedAge:days]`. |
...
* **A long grace period keeps a finished event in the guide as if it were still to come.** Because Dispatcharr repeats a dateless event every day, a channel that stays visible after its event shows `Upcoming at <time>` again the next day. A grace of several days, for example `[UndatedEnded:96]` together with `[UndatedAge:4]`, is useful when a provider publishes a weekend slate on the Thursday before, because the name gives no way to tell a Friday event from a Saturday one. The cost is that Friday's finished events stay visible, listed as upcoming, until the grace runs out.
* **A name that states a day of the week is anchored to that day.** A full day name, or the football markers `MNF`, `TNF` and `SNF`, is resolved to the first such day on or after the date the channel was first seen. A provider that publishes a weekend slate several days early would otherwise have every one of its events judged against the day the channels appeared. The day is read from the name and never guessed from its absence, and it only ever moves the window later, never earlier. No short form is read at all, so `Wed`, `Thu` and the rest anchor nothing: `Sat` and `Sun` appear inside ordinary names such as the broadcaster SAT.1 and the team Arizona State Sun Devils, and a day taken from an ordinary word would move the window rather than widen it, so the whole set is left out rather than some of it. A full day name is read wherever it appears, including inside a programme title, so a channel called `Saturday Night Live 11:30pm` is anchored to Saturday. That can only keep such a channel visible longer, never hide it sooner, and `[UndatedAge:days]` remains the outer bound.
...
* **A channel that appears after its inferred window has already closed is left visible.** A channel first seen at 23:00 and named for a 1:00am event is named for the next 1:00am, not the one that ended eighteen hours earlier, so the rule declines to act rather than hiding a channel whose event has not started. This needs the first-seen moment, which the plugin began recording alongside the date in this version; a record written by an earlier version carries the date only and does not get the check until its channel name next changes. A channel whose name states a day is checked against the window for that day, so a channel that used to be left visible for ever by this check can now be hidden once its real event has ended.
...
Three details that answer most questions about it:

* **Only channels that would otherwise be visible are compared.** A channel already hidden by a hide rule is not a duplicate of anything, so a run in which every channel is hidden reports 0 duplicates.
* **A duplicate hide carries no hide rule.** Its CSV `reason` reads `[Duplicate] Another channel has the same event and was kept`, and the header counts it under **Duplicates Hidden**. If you show one of these channels by hand, the next scan hides it again. Look for the same event on the other channel number, or turn on **Keep Duplicate Channels**.
* **The plugin hides channels; it never deletes them.** A hidden channel leaves the profile, and so the lineup and guide for that profile, but it is still listed in Dispatcharr. Delete unwanted copies in Dispatcharr's own channel list.
...
**The guide shows yesterday's game on a channel whose stream now carries today's.** Many providers reuse the same numbered streams every day and rename them. A channel you created by hand keeps the name it was given, so its guide entry, and this plugin's date rules under `Channel_Name`, keep reading the old game. With **Auto Channel Sync** enabled for the group, Dispatcharr renames each channel it created when the stream's name changes (checked in Dispatcharr 0.31.0), which keeps the channel name current. If you turn it on, also enable **🔄 Auto-rescan after M3U refresh**, because each sync turns every channel in the group back on.
...
**Date format inside the `Upcoming at` title** follows your existing `Date Format` setting:
...
* **A regex field I filled in seems to do nothing**: The CSV header's **Regex Field Matches** block counts what each one matched, and a field that matched zero channels is called out in the result message. The three regex fields are matched against the channel or stream name only, never against guide programme titles, so text copied out of the TV Guide will never match. Filler text such as a "guide information is currently unavailable" line is a programme description generated by Dispatcharr, not a channel name. Characters such as `[ ] ( ) | . +` have a special meaning in a regular expression, so a name pasted in as it is may not match itself: `ESPN [HD]` does not match the name `ESPN [HD]`, while `ESPN \[HD\]` or plain `ESPN` does.
* **I set Force Visible to one channel and the others stay visible**: **✅ Regex: Force Visible Channels** keeps the channels it matches visible and does nothing to any other channel. Every channel it does not match still goes through **Hide Rules Priority**, so it cannot be used as a list of the only channels to show. To hide the rest, use **💤 Regex: Mark Channel as Inactive** with the `[InactiveRegex]` rule, or the other hide rules.
* **A channel with a blank name stays visible**: `[BlankName]` hides a name that is empty or only spaces, but only when it is in **Hide Rules Priority**. With **Name Source** set to `Stream_Name` it checks the first stream's name instead, so a blank channel whose stream has a name is not seen as blank.
* **A log line says "No match found"**: this plugin does not write that message, so it comes from another plugin or from Dispatcharr itself. When a regex field matches nothing, this plugin logs `'Regex: Force Visible Channels' is set but matched no channels this scan` (or the same line naming the field that matched nothing).
* **Channels named `NO EVENT`, or with nothing after the colon, are never hidden**: check **Name Source**. With `Stream_Name` every rule reads the name of the first stream, not the channel name you see, so a placeholder that exists only in the channel name is invisible to `[NoEventPattern]` and `[EmptyPlaceholder]`. Switch to `Channel_Name` and compare the two with a **Dry Run**.
* **Cancelled events stay visible**: no rule reads the word. Put `CANCELL?ED` in **💤 Regex: Mark Channel as Inactive** and keep `[InactiveRegex]` in **Hide Rules Priority**.
* **Can a channel be hidden while its guide shows a particular programme, such as "Pause"?** Not at present. The hide rules read the channel or stream name only. `[NoEPG]`, the one rule that looks at the guide, checks only whether the channel has any programme in the next 24 hours and never reads a title. A rule of that kind could also act only when a scan runs, so it would follow the guide only as closely as your scan schedule.
...
* **Every channel in my groups is hidden and the lineup is empty**: open the newest CSV and count the `hide_rule` column. When the provider is publishing only placeholders (`[EmptyPlaceholder]`, `[NoEventPattern]`) and finished events (`[PastDate]`), an empty lineup is the correct result, and channels come back as the provider names them for new events. Also check the header for **Channel Groups that matched no channels**, which means a group name matches nothing in the profile.
* **Nothing runs on a schedule**: an empty **Scheduled Run Times** box means only **Run Now**, **Dry Run** and the **Auto-rescan after M3U refresh** ever run the scan.
...
* **Guide shows literal `{month}/{day}` or `{starttime}` in the programme title** (e.g. `GOBI Live From Coachella 2026 {month}/{day} {starttime} CDT`): **fixed.** This hit *matched* event channels whose name has no parseable date **and** time (a bare year like `2026` is not a date). The timezone-localized title template embedded those placeholders, and the renderer leaves any placeholder it can't fill as literal text. ECM now uses a plain `{title}` for the live title, and timed channels still land in the correct, timezone-converted guide slot, and the `Upcoming at` title keeps the localized date and time (it only renders when a date and time were parsed). The `Ended at` title carries the end time only, see the next entry. A related fix lets the event-number separator be `-` (e.g. `LIVE EVENT 31 - GOBI ...`) so the leading `- ` no longer leaks into the title. Re-run a scan and refresh your EPG if you still see the old behavior.
* **The `Ended at` title shows the day before the event ended** (e.g. `Ended at 9/19 12 AM CDT` for an event that ended at midnight on 9/20): **fixed in v1.26.2621024.** Dispatcharr fills `{month}` and `{day}` with the date the event started and has no placeholder for the date it ended, so any event running past midnight got the wrong date. The `Ended at` title now carries the end time and zone only, for example `Ended at 12 AM CDT: RAF 13`. Update the plugin and run one applied scan (not a dry run), which rewrites the managed source's templates.
* **The EPG column reads Not Assigned on hidden channels**: expected with **🔌 Auto-Remove EPG on Hide** on, which removes the guide binding when a channel is hidden, and the managed dummy is attached only to visible channels. Once a channel stays visible, the next scan attaches the managed dummy again if **Manage Dummy EPG** is on.
